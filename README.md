...
## Prerequisites
A Ledger Compliance instance must be up and running. You need IP address, port (usually 443) of the Ledger Compliance grpc service.
...
```make init``` inside **src** directory to install (via pip) all the needed dependencies, followed by a ```make install``` to install the library.
